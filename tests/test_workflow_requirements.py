...
from src.workflows.orchestrate_prod import _ensure_anonymized_dataset_marker, _prediction_is_current, _top_k_failure
...
def test_phase_completion_triggers_reporting_and_final_production() -> None:
    ...
    post_lock = Path(".github/workflows/post-lock-validation.yml").read_text(encoding="utf-8")
    ...
    assert "rm -rf experiments prod docs" in pages
    ...
    assert "rm -rf experiments prod docs" in readme
    assert 'git ls-files -- "$path"' in readme
    assert 'git add -A "$path"' in readme
    ...
    assert "--post-lock-only" in post_lock
    assert "--github-branch experiments" in post_lock
    assert 'dispatch_optional pages-update.yml' in post_lock
    assert 'dispatch_optional readme-update.yml' in post_lock
...
def test_final_post_lock_uses_full_selected_recipe_decisions(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    class FakePipeline:
        ...
        def _evaluate_post_lock_sliding_windows(**kwargs):
            ...
            return {
                ...
                "models": {
                    "A": {
                        "record_public_id": "E_1",
                        "windows_completed": 10,
                        "window_metrics_summary": {"direction_accuracy": {"min": 0.7, "avg": 0.8, "max": 0.9}},
                        "long_run_metrics": {"direction_accuracy": 0.8},
                    }
                },
            }
    ...
    phase_path = tmp_path / "experiments" / "btc_1h" / "results" / "phase_results_public.json"
            ...
            [
                {
                    ...
                    "validation": {"direction_accuracy": 0.8},
                }
            ]
        ),
        encoding="utf-8",
    ...
    assert rows[0]["post_lock_sliding_summary"]["model_public_id"] == "A"


def test_final_post_lock_rejects_incomplete_top_k_model(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    class FakePipeline:
        @staticmethod
        def _evaluate_post_lock_sliding_windows(**kwargs):
            return {
                ...
                "models": {
                    "A": {
                        ...
                        "long_run_metrics": {"direction_accuracy": 0.0},
                    }
                },
            }
...
def test_post_lock_rerun_prefers_existing_selected_top_k() -> None:
    records = [
        {
            "public_id": "E_unselected_high",
            "selected_for_next_phase": False,
            "status": "worked",
            "validation": {"direction_accuracy": 0.99, "balanced_accuracy": 0.99},
        },
        {
            "public_id": "E_selected_a",
            "selected_for_next_phase": True,
            "status": "worked",
            "validation": {"direction_accuracy": 0.70, "balanced_accuracy": 0.70},
        },
        {
            "public_id": "E_selected_b",
            "selected_for_next_phase": True,
            "status": "worked",
            "validation": {"direction_accuracy": 0.69, "balanced_accuracy": 0.69},
        },
    ]

    selected = _post_lock_selected_records(records, ranking_metric="direction_accuracy", top_k=2)

    assert [record["public_id"] for record in selected] == ["E_selected_a", "E_selected_b"]
...
def test_per_market_configs_ignore_stale_legacy_root_config(tmp_path: Path) -> None:
    configs = tmp_path / "requests" / "configs"
    configs.mkdir(parents=True)
    (configs / "btc.yaml").write_text(dump_yaml(_base_config("btc_1h", "done")), encoding="utf-8")
    (tmp_path / "requests" / "config.yaml").write_text(dump_yaml(_base_config("btc_1h", "none")), encoding="utf-8")

    assert [path.name for path in _config_files(tmp_path)] == ["btc.yaml"]
    assert _first_incomplete_config(tmp_path) is None
...
def test_prod_top_k_failure_is_parsed_as_blocking_precondition() -> None:
    message = "Production requires exactly 3 Top K models; found 1 eligible models"

    assert _top_k_failure(message) == {"requested": 3, "available": 1}
    assert _top_k_failure("some other production failure") is None
...
def test_update_pages_generates_docs_from_public_artifacts(tmp_path: Path) -> None:
    ...
    eth_exp_results = tmp_path / "experiments" / "eth_1h" / "results"
    prod_results = tmp_path / "prod" / "btc_1h" / "results"
    exp_results.mkdir(parents=True)
    eth_exp_results.mkdir(parents=True)
        ...
        phase_rows.append(
            {
                ...
                "post_lock_sliding_summary": {
                    ...
                    "long_run_metrics": {"balanced_accuracy": 0.66, "direction_accuracy": 0.67},
                },
            }
        )
    (exp_results / "phase_results_public.json").write_text(json.dumps(phase_rows), encoding="utf-8")
    (eth_exp_results / "run_manifest_public.json").write_text(
        json.dumps({"market_id": "eth_1h", "workflow_profile": "exhaustive_v1", "status": "completed"}),
        encoding="utf-8",
    )
    (eth_exp_results / "candidates_public.json").write_text(json.dumps([]), encoding="utf-8")
    (eth_exp_results / "phase_results_public.json").write_text(
        json.dumps(
            [
                {
                    "experiment_id": "E_eth",
                    ...
                    "status": "worked",
                    "selected_for_next_phase": True,
                    "train": {"balanced_accuracy": 0.61},
                    "validation": {"balanced_accuracy": 0.62, "direction_accuracy": 0.63},
                    "test": {"balanced_accuracy": 0.64, "direction_accuracy": 0.65},
                    "overfit_diagnostics": {"train_valid_gap": -0.01},
                    "runtime_seconds": 1.0,
                }
            ]
    ...
    assert "Experiment markets: 2" in index
    ...
    assert "BTC 1H Experiment Progress" in experiments_html
    assert "ETH 1H Experiment Progress" in experiments_html
    assert "BTC 1H Post-Lock Sliding Validation" in experiments_html
    ...
    assert "ETH 1H: 1/16" in experiments
...
def test_reset_all_outputs_dry_run_deletes_nothing(tmp_path: Path) -> None:
    ...
    assert result["branch_resets"] == [
        ...
        {"branch": "prod", "files": ["README.md"], "status": "planned"},
...
def test_reset_all_outputs_force_resets_artifact_branches_without_touching_main_checkout(tmp_path: Path) -> None:
    ...
    assert result["deleted_local_paths"] == []
    assert calls == [(tmp_path, "experiments", False), (tmp_path, "prod", False), (tmp_path, "gh-pages", False)]
...
def test_artifact_branch_restore_keeps_prior_phase_rows_available(tmp_path: Path) -> None:
    origin = tmp_path / "origin.git"
    repo = tmp_path / "repo"
    ...
    artifact = repo / "experiments" / "btc_1h" / "results" / "phase_results_public.json"
    artifact.parent.mkdir(parents=True)
    artifact.write_text(json.dumps([{"phase_id": "phase07", "experiment_id": "old"}]), encoding="utf-8")
    git(repo, "add", "-f", "experiments")
    git(repo, "commit", "-m", "Artifact rows")
    git(repo, "push", "origin", "HEAD:experiments")
    shutil.rmtree(repo / "experiments")

    result = hf_state.restore_artifact_branch_paths(
        repo,
        paths=["experiments/btc_1h"],
        target_branch="experiments",
    )
    ...
    assert restored == [{"phase_id": "phase07", "experiment_id": "old"}]


def test_experiment_report_orders_phases_numerically() -> None:
        ...
        phase_rows=[
            {
                ...
                "experiment_id": "ten",
                "selected_for_next_phase": True,
                "validation": {"balanced_accuracy": 0.1, "direction_accuracy": 0.1},
                "test": {},
                "overfit_diagnostics": {},
            },
            {
                ...
                "experiment_id": "two",
                "selected_for_next_phase": True,
                "validation": {"balanced_accuracy": 0.2, "direction_accuracy": 0.2},
                "test": {},
                "overfit_diagnostics": {},
            },
        ],
    ...
    assert report.index("## Phase 2") < report.index("## Phase 10")
