...
def restore_artifact_branch_paths(root: Path, *, paths: list[str], target_branch: str | None) -> dict[str, Any]:
    if not target_branch:
        return {"status": "skipped", "reason": "no target branch", "paths": []}
    requested = [_safe_relative_path(path) for path in paths]
    tmp_parent = root / "tmp"
    tmp_parent.mkdir(parents=True, exist_ok=True)
    restored: list[str] = []
    with tempfile.TemporaryDirectory(prefix=f"artifact-restore-{target_branch.replace('/', '-')}-", dir=tmp_parent) as tmp:
        ...
        fetch = subprocess.run(["git", "fetch", "--depth=1", "origin", target_branch], cwd=artifact_repo)
        if fetch.returncode != 0:
            return {"status": "skipped", "reason": "target branch not found", "paths": []}
        subprocess.run(["git", "checkout", "--detach", "FETCH_HEAD"], cwd=artifact_repo, check=True)
        for relative in requested:
            source = artifact_repo / relative
    ...
    return {"status": "restored" if restored else "skipped", "paths": restored}
