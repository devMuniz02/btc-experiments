...
def _phase_groups(experiment: dict[str, Any]) -> list[tuple[str, list[dict[str, Any]]]]:
    ...
    if experiment["workflow_profile"] == "exhaustive_v1":
        return [(phase, by_phase.get(phase, [])) for phase in PUBLIC_DISCOVERY_PHASES]
    return [
        (phase, group)
...
def _phase_status(experiment: dict[str, Any]) -> str:
    ...
    if experiment["workflow_profile"] == "exhaustive_v1":
        ...
        return "Complete" if complete == len(PUBLIC_DISCOVERY_PHASES) else f"{complete}/{len(PUBLIC_DISCOVERY_PHASES)}"
...
def _overview(experiments: list[dict[str, Any]], production: list[dict[str, Any]]) -> str:
    ...
    total = sum(len(_phase_groups(item)) for item in experiments)
...
def _post_lock_sliding_table(experiment: dict[str, Any]) -> str:
    ...
    rows_with_summary = [row for row in phase16_rows if row.get("post_lock_sliding_summary")]
    ...
    if not rows:
        return ""
    return _section("Post-Lock Sliding Validation", subtitle="Out-of-sample 300-row test windows before the production anchor, plus first-window long-run test metrics.") + _table(
        [
            "Model",
            "Windows",
            "Min Direction",
...
def _experiments_page(experiments: list[dict[str, Any]]) -> str:
    if not experiments:
        ...
        for index, experiment in enumerate(experiments):
            blocks.append(
                f'<section class="market-block">{_section("Experiment market", _status(_phase_status(experiment)))}'
