...
def _config_files(cache_root: Path) -> list[Path]:
    candidates = list((cache_root / REQUEST_ROOT / "configs").glob("*.yaml"))
...
def _run_final_post_lock_sliding_validation(
    *,
    ...
    summaries_by_public_id = {
        str(model.get("record_public_id") or ""): {
            "windows_completed": model.get("windows_completed"),
            "window_metrics_summary": model.get("window_metrics_summary"),
            "long_run_metrics": model.get("long_run_metrics"),
        }
        for model in (validation.get("models") or {}).values()
    }
    ...
    if phase_path.exists():
        ...
        selected_ids = {str(record.get("public_id") or "") for record in selected}
        for row in rows:
